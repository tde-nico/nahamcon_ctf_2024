...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m1709constructorimpl = SegmentOrClosed.m1709constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1709constructorimpl = SegmentOrClosed.m1709constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1714isClosedimpl(m1709constructorimpl)) {
                Segment m1712getSegmentimpl = SegmentOrClosed.m1712getSegmentimpl(m1709constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m1712getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m1712getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m1712getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m1712getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1712getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1712getSegmentimpl(m1709constructorimpl);
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (segment.getId() >= m1712getSegmentimpl.getId()) {
                    ...
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, m1712getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1712getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
