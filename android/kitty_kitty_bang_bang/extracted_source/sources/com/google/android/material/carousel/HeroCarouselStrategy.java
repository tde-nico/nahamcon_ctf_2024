...
public class HeroCarouselStrategy extends CarouselStrategy {
    ...
    public KeylineState onFirstChildMeasuredWithMargins(Carousel carousel, View view) {
        ...
        int[] iArr2 = f2 < 2.0f * smallSizeMin ? new int[]{0} : SMALL_COUNTS;
        int max = (int) Math.max(1.0d, Math.floor((f2 - (CarouselStrategyHelper.maxValue(iArr) * smallSizeMax)) / min));
        ...
        Arrangement findLowestCostArrangement = Arrangement.findLowestCostArrangement(f2, clamp, smallSizeMin, smallSizeMax, i3 != 0 ? doubleCounts(iArr2) : iArr2, f3, i3 != 0 ? doubleCounts(MEDIUM_COUNTS) : MEDIUM_COUNTS, min, iArr3);
        this.keylineCount = findLowestCostArrangement.getItemCount();
        if (findLowestCostArrangement.getItemCount() > carousel.getItemCount()) {
            findLowestCostArrangement = Arrangement.findLowestCostArrangement(f2, clamp, smallSizeMin, smallSizeMax, iArr2, f3, MEDIUM_COUNTS, min, iArr3);
            i = 0;
        } else {
            i = i3;
        }
